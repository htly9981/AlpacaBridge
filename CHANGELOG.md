...
- **QHY camera: exposure transfer duration logging** (AlpacaCore `qhy_camera_driver.cpp`): the exposure worker thread now logs how long `GetQHYCCDSingleFrame` actually took (INFO on success, WARN on failure) instead of only recording success/failure. The exposure watchdog (`get_camera_state()`) already marks a slow exposure `Failed` after exposure duration + 60s and returns control to the client, but the SDK call keeps running in the background and silently overwrites the state once it finishes — until now there was no record of how long that took, making it impossible to tell a genuinely hung SDK call from one that was merely slower than the margin.
